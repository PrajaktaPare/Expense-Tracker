...
export default function Overview() {
  ...
  const todayExpenses = getTodayExpenses(userExpenses)
  const totalSpent = calculateTotalExpenses(currentMonthExpenses)
  const expensesByCategory = calculateExpensesByCategory(currentMonthExpenses, userCategories)
  const monthlyData = getMonthlyData(userExpenses)

  // Calculate budget progress
  const budgetProgress = userBudget > 0 ? Math.min(100, (totalSpent / userBudget) * 100) : 0
  ...
  const recentTransactions = [...userExpenses]
    .filter((exp) => exp && exp.date) // Filter out invalid expenses
    .sort((a, b) => new Date(b.date) - new Date(a.date))
    .slice(0, 5)

  // Prepare data for pie chart
  const pieChartData = userCategories
    .map((category) => ({
      id: category.id,
      name: category.name,
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white rounded-xl shadow-sm p-6 border border-purple-100">
          ...
          <div className="text-sm text-gray-500">Budget: {formatCurrency(userBudget)}</div>
          ...
          <div className="mt-4 text-xs">
            {todayExpenses.length > 0 ? (
              <div className="bg-purple-50 p-2 rounded-lg">
                ...
                {(() => {
                  const biggestExpense = todayExpenses.sort((a, b) => b.amount - a.amount)[0]
                  const category = userCategories.find((c) => c.id === biggestExpense?.categoryId)
                  return (
                    <p className="mt-1">
                      {category?.name || "Unknown"}: {formatCurrency(biggestExpense?.amount || 0)}
                    </p>
                  )
                })()}
              </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 border border-purple-100">
          ...
          <div className="text-3xl font-bold text-gray-800 mb-2">{userExpenses.length}</div>
          ...
          <div className="mt-4 text-xs">
            <div className="bg-purple-50 p-2 rounded-lg">
              ...
              <p className="mt-1">
                {formatCurrency(
                  userExpenses.length > 0 ? calculateTotalExpenses(userExpenses) / userExpenses.length : 0,
                )}
              </p>
            </div>
          </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 border border-purple-100">
          ...
          <div className="h-64">
            {pieChartData.length > 0 ? (
              ...
              <div className="flex items-center justify-center h-full text-gray-500">No expenses to display</div>
            )}
          </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 border border-purple-100">
          ...
          <div className="space-y-4">
            {topCategories.length > 0 ? (
                  ...
                  <div className="flex-1">
                    ...
                    <div className="w-full h-2 bg-gray-100 rounded-full">
                      <div
                        ...
                        style={{
                          width: `${Math.min(100, (category.spent / (category.budget || 1)) * 100)}%`,
              ...
              <div className="text-gray-500 text-center py-4">No spending data available</div>
            )}
          </div>
        </div>
          ...
          <div className="space-y-4">
            {recentTransactions.length > 0 ? (
              recentTransactions.map((transaction) => {
                ...
                return (
                  <div key={transaction.id || transaction._id} className="flex items-center">
                    <div
                      className="w-10 h-10 rounded-lg flex items-center justify-center mr-4"
                    ...
                    <div className="flex-1">
                      <div className="flex justify-between">
                        <div>
                          <div className="font-medium text-gray-800">{category?.name || "Unknown Category"}</div>
                          <div className="text-xs text-gray-500">{transaction.description || "No description"}</div>
                        </div>
